...
class ModuleNewsletterList extends Module
{
	...
	protected function compile()
	{
		...
		while ($objNewsletter->next())
		{
			...
			{
				$objJumpTo = $this->Database->prepare("SELECT id, alias FROM tl_page WHERE id=? AND (start=? OR start<?) AND (stop=? OR stop>?) AND published=?")
											->limit(1)
