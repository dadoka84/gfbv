...
<input type="submit" name="test" class="tl_submit" alt="test newsletter" accesskey="t" value="'.specialchars(sprintf($GLOBALS['TL_LANG']['tl_newsletter']['test'], $objNewsletter->sender)).'" /> 
	...
	public function getSearchablePages($arrPages, $intRoot=0)
	{
		...
		while ($objNewsletter->next())
		{
			...
			$objParent = $this->Database->prepare("SELECT id, alias FROM tl_page WHERE id=? AND (start=? OR start<?) AND (stop=? OR stop>?) AND published=?")
										->limit(1)
										->execute($objNewsletter->jumpTo, '', time(), '', time(), 1);
			...
			// Get items
			$objItem = $this->Database->prepare("SELECT * FROM tl_newsletter WHERE pid=? AND sent=? ORDER BY date DESC")
