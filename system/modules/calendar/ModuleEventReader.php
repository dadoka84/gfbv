...
class ModuleEventReader extends Events
{
	...
	protected function compile()
	{
		...
		// Get current event
		$objEvent = $this->Database->prepare("SELECT *, (SELECT title FROM tl_calendar WHERE tl_calendar.id=tl_calendar_events.pid) AS calendar FROM tl_calendar_events WHERE pid IN(" . implode(',', $this->cal_calendar) . ") AND (id=? OR alias=?)" . (!BE_USER_LOGGED_IN ? " AND published=?" : ""))
								   ->limit(1)
				...
				for ($i=0; $i<count($arrEnclosure); $i++)
				{
					...
					{				
						...
						if (in_array($objFile->extension, $allowedDownload))
						{
							...
							$arrEnclosures[$i]['href'] = $this->Environment->request . (($GLOBALS['TL_CONFIG']['disableAlias'] && $this->Input->get('id')) ? '&amp;' : '?') . 'file=' . $this->urlEncode($arrEnclosure[$i]);
