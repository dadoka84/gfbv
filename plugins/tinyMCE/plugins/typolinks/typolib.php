...
class typolib extends Controller
{
	...
	public function createFileList($strFolder=null, $level=-1)
	{
		...
		foreach ($arrPages as $strFile)
		{
			...
			{
				$strFiles .= sprintf('<option value="%s">%s%s</option>', $strFolder . '/' . $strFile, str_repeat("&nbsp;", (2 * $level)), specialchars($strFile));
			}
		}
	...
	public function createImageList($strFolder=null, $level=-1)
	{
		...
		foreach ($arrPages as $strFile)
		{
			...
			{
				$strFiles .= sprintf('["%s%s", "%s"]', str_repeat("&nbsp;", (2 * $level)), specialchars($strFile), $strFolder . '/' . $strFile) . ",\n";
			}
		}
	...
	public function createTemplateList()
	{
		...
		foreach ($arrTemplates as $strFile)
		{
			...
			{
				$strFiles .= sprintf('["%s", "../' . $GLOBALS['TL_CONFIG']['uploadPath'] . '/tiny_templates/%s", "' . $GLOBALS['TL_CONFIG']['uploadPath'] . '/tiny_templates/%s"]', $strFile, $strFile, $strFile) . ",\n";
			}
		}
