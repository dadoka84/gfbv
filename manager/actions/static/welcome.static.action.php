...
?>
<div class="subTitle">
	<span class="right"><img src="media/images/_tx_.gif" width="1" height="5"><br /><?php echo $site_name ;?> - <?php echo $_lang["home"]; ?></span>
</div>

<div class="sectionHeader"><img src='media/images/misc/dot.gif' alt="." />&nbsp;<?php echo $_lang["welcome_title"]; ?></div><div class="sectionBody">
...
?>
  ...
  <tr>
	<td colspan="2">
		<i><a href="index.php?a=10"><img src="media/images/icons/messages.gif" align="absmiddle" border=0></a>&nbsp;<?php printf($_lang["welcome_messages"], $_SESSION['nrtotalmessages'], $_SESSION['nrnewmessages']) ?></i>
	</td>
  </tr>
		<?php } ?>
</table>
...
<div class="sectionHeader"><img src='media/images/misc/dot.gif' alt="." />&nbsp;<?php echo $_lang["onlineusers_title"]; ?></div><div class="sectionBody">
	...
	if($limit<1) {
		...
		for ($i = 0; $i < $limit; $i++) { 
			...
			echo "<tr><td><b>".$activeusers['username']."</td><td>".$activeusers['internalKey']."</td><td></b>".$activeusers['ip']."</td><td>".strftime('%H:%M:%S', $activeusers['lasthit']+$server_offset_time)."</td><td>$currentaction</td></tr>";
		}
	}
	?> 
	</tbody>   
