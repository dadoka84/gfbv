...
class InstallTool extends Controller
{
	...
	public function run()
	{
		...
		{
			$password =  sha1($this->String->decodeEntities($this->Input->post('password')));
		...
		if ($this->Input->post('FORM_SUBMIT') == 'tl_install')
		{
			...
			{
				$this->Config->update("\$GLOBALS['TL_CONFIG']['installPassword']", sha1($this->String->decodeEntities($this->Input->post('password'))));
				$this->reload();
			}
		}
		...
		if ($this->Input->post('FORM_SUBMIT') == 'tl_database_login')
		{
			...
			{
				$this->Config->update("\$GLOBALS['TL_CONFIG']['$strKey']", $this->String->decodeEntities($this->Input->post($strKey)));
			}
		...
		if ($this->Input->post('FORM_SUBMIT') == 'tl_admin')
		{
			...
			elseif(strlen($this->Input->post('name')) && strlen($this->Input->post('email')) && strlen($this->Input->post('username')))
			{
				$this->Database->prepare("INSERT INTO tl_user (tstamp, name, email, username, password, admin, showHelp, useRTE, thumbnails) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
							   ->execute(time(), $this->Input->post('name'), $this->Input->post('email'), $this->Input->post('username'), sha1($this->String->decodeEntities($this->Input->post('pass'))), 1, 1, 1, 1);
